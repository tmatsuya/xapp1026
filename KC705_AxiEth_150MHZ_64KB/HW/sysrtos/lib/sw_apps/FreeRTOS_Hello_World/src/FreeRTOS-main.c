...
static int end_criterion=0;
	/* Initialise xNextWakeTime - this only needs to be done once. */
	xNextWakeTime = xTaskGetTickCount();

	for( ;; )
	{
		xil_printf("Hello from prvQueueSendTask\n\r");
		xil_printf("*********************************\n\r");
		...
		end_criterion++;
		if (end_criterion==2){
			xil_printf("We have demonstrated the scheduling of task in freeRTOS ...\nexiting from prvQueueSendTask ...\nGood Bye\n\r");
			exit(-1);
		}
...
static int end_criterion=0;
	for( ;; )
	{
		xil_printf("Hello from prvQueueReceiveTask\n\r");
		xil_printf("*********************************\n\r");
